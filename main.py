...
REQUEST_DELAY = 2
TIMESLOT_URL = (
    "https://ttp.cbp.dhs.gov/schedulerapi/slots?orderBy=soonest&limit={limit}&locationId={location_id}&minimum=1"
)
...
def import_mapping_from_url() -> dict:
    ...
    return {
        location["id"]: "{} ({}, {})".format(location["name"], location["city"], location["state"])
...
def get_timeslots_for_location_ids(location_ids: list, before: Union[str, None] = None, limit: int = 10) -> dict:
...
def generate_notification_texts(location_mapping: dict, all_timeslots: dict, silent: bool) -> List:
    ...
    for location_id, timeslots in all_timeslots.items():
        if len(timeslots) > 0:
            location_texts = []
            location_texts.append("{} ({})".format(location_mapping[location_id], location_id))
            for timeslot in timeslots:
                location_texts.append(
                    "    {}".format(timeslot.strftime("%B %d, %Y (%a) @ %I:%M %p").replace(" 0", " "))
                )
...
def send_to_discord(notification_texts: List[str]) -> None:
...
if __name__ == "__main__":
    ...
    notification_text = generate_notification_texts(location_mapping, all_timeslots, args["silent"])
